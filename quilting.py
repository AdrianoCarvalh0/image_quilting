...
def l2_top_bottom(overlap_size, patch_top, patch_bottom):
...
def l2_left_right(overlap_size, patch_left, patch_right):
...
def synth_texture(src_texture, blk_size, overlap_size):
    ...
    target_height = h_new
    target_width = w_new
    target = np.zeros((target_height, target_width, c), dtype=src_texture.dtype)
    ...
    for y in range(ny_blocks):
        ...
        for x in range(nx_blocks):
            ...
            if y == 0:
                ...
                left_right_join = compute_lr_join(left_block, best_right_block, blk_size, overlap_size)
                ...
                if x == 0:
                    ...
                    top_bottom_join = compute_bt_join(top_block, best_bottom_block, blk_size, overlap_size)
                    full_join = np.vstack(
...
def synth_texture_rand(texture, blk_size):
    ...
    blocks = np.array([texture[y:y + blk_size, x:x + blk_size] for x, y in ind])
...
def synth_texture_neighborhood(texture, blk_size, overlap_size):
    h, w, c = texture.shape
    ...
    xs = np.arange(x_max)
    ys = np.arange(y_max)
    all_blocks = np.array([texture[y:y + blk_size, x:x + blk_size] for x in xs for y in ys])

    target_height = h_new
    target_width = w_new
    target = np.zeros((target_height, target_width, c), dtype=texture.dtype)
